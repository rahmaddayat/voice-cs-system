...
def normalize_numbers(text: str) -> str:
    ...
    def replace_num(match):
        raw = match.group()
        try:
            # Angka desimal (gunakan titik sebagai pemisah)
            cleaned = raw.replace(',', '.')
            if '.' in cleaned:
            ...
            return raw
    # Cocokkan angka bulat dan desimal (dengan koma/titik)
    return re.sub(r'\b\d+[.,]?\d*\b', replace_num, text)

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Menyelaraskan folder dengan petunjuk sebelumnya (coqui_tts)
COQUI_DIR = os.path.join(BASE_DIR, "coqui_tts")

# [SOLVED] Melengkapi jalur path ke file model TTS Anda
...
def _tts_with_coqui(text: str) -> str:
    ...
    # Definisikan jalan ke file speakers.pth di dalam folder coqui_tts sesuai modul
    ...
    cmd = [
        ...
        "--speakers_file_path", COQUI_SPEAKERS_PATH,  # <--- KITA PAKSA JALUR DI SINI
...
if __name__ == "__main__":
    # Blok kode testing mandiri untuk mengecek apakah TTS berjalan dengan baik
